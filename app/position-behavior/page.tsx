...
export default function PositionBehavior() {
  ...
  const [autoShowOnScroll, setAutoShowOnScroll] = useState(false)
  ...
  useEffect(() => {
    ...
    localStorage.setItem("qrCodePositionSettings", JSON.stringify(settings))
  }, [position, marginSize, marginX, marginY, startCollapsed, autoShowOnScroll, animation])

  // Load settings from localStorage on component mount
  useEffect(() => {
    const savedSettings = localStorage.getItem("qrCodePositionSettings")

    if (savedSettings) {
      const settings = JSON.parse(savedSettings)
      setPosition(settings.position || "bottom-right")
      setMarginSize(settings.marginSize || "medium")
      setMarginX(settings.marginX || "20")
      setMarginY(settings.marginY || "20")
      setStartCollapsed(settings.startCollapsed !== undefined ? settings.startCollapsed : true)
      setAutoShowOnScroll(settings.autoShowOnScroll || false)
      setAnimation(settings.animation || "fade")
    }
  }, [])
  ...
  return (
    <div className="flex min-h-screen bg-[#e9ede7]">
            ...
            <div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    id="auto-show"
                    checked={autoShowOnScroll}
                    onCheckedChange={setAutoShowOnScroll}
                    className="data-[state=checked]:bg-black"
                  />
                </div>
      ...
      {/* Preview Panel */}
      <div className="w-80 p-6 bg-white rounded-l-3xl">
          ...
          <div className="flex flex-col items-center">
            <div className="bg-white p-6 rounded-3xl shadow-sm mb-4">
              <div className="w-48 h-48 bg-white flex items-center justify-center">
                <QRCodeDisplay url="https://example.com" size={180} />
              </div>
        ...
        <div className="mt-8">
          ...
          <div className="space-y-3 text-base">
            ...
            <div className="flex justify-between">
              ...
              <span className="truncate max-w-[180px] font-medium">Current Page</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500">Auto-show on scroll:</span>
              <span className="font-medium">{autoShowOnScroll ? "Yes" : "No"}</span>
            </div>
          </div>
        </div>
      </div>

      {/* Live Demo of Floating QR Code with current settings */}
      <FloatingQRCode
        position={position as any}
        showOnScroll={autoShowOnScroll}
