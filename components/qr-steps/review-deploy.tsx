...
export default function ReviewDeploy({ state, updateState, onPrevious, onSave }) {
  ...
  const handleDeploy = () => {
    ...
    // After 3 seconds, redirect to the QR codes list
    setTimeout(() => {
  ...
  return (
    <CreateQRLayout
      ...
      additionalInfo={{
        Status: isDeployed ? "Deploying..." : "Ready to deploy",
      }}
    >
  ...
  data-qr-id="${state.qrCodeName.toLowerCase().replace(/\s+/g, "-")}"
